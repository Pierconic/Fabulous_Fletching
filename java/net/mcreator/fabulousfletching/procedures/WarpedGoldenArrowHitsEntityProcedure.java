...
public class WarpedGoldenArrowHitsEntityProcedure {
		...
		if (!(sourceentity == entity)) {
			...
			if (Math.random() < 0.3) {
				if (!((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(world.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.MULTISHOT)) != 0)
					...
					if (world instanceof ServerLevel _level) {
						ItemEntity entityToSpawn = new ItemEntity(_level, (sourceentity.getX()), (sourceentity.getY()), (sourceentity.getZ()), new ItemStack(FabulousfletchingModItems.WARPED_GOLDEN_ARROW.get()));
