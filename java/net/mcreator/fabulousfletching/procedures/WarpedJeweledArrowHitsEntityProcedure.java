...
public class WarpedJeweledArrowHitsEntityProcedure {
		...
		if (!(entity == sourceentity)) {
			...
			if (Math.random() < 0.8) {
				if (!((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(world.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.MULTISHOT)) != 0)
					...
					if (world instanceof ServerLevel _level) {
						ItemEntity entityToSpawn = new ItemEntity(_level, (sourceentity.getX()), (sourceentity.getY()), (sourceentity.getZ()), new ItemStack(FabulousfletchingModItems.WARPED_JEWELED_ARROW.get()));
